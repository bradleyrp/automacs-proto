...
>>>>>>> 3ab7135935efee2758a57569f35af9ced5498398
		...
		if not os.path.isfile(self.rootdir+'solvate-minimized.gro'): 
			...
			#---sometimes minimization causes the lipids to jump across the box
			cmd = [gmxpaths['trjconv'],
				'-f vacuum-minimized.gro',
				'-o vacuum-minimized-coherent.gro',
				'-s em-vacuum-steep.tpr',
				'-pbc nojump']
			call(cmd,logfile='log-trjconv-solvate-coherent',cwd=self.rootdir,inpipe="0\n")				
			
			print "checking the size of the box"
			cmd = [gmxpaths['editconf'],
				'-f vacuum-minimized-coherent.gro',
			...
			with open(self.rootdir+'vacuum-minimized-coherent.gro','r') as fp: gro1 = fp.readlines()
